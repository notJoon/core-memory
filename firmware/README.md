...
> This is a fork of [mcbridejc/core-mem-controller](https://github.com/mcbridejc/core-mem-controller).
> The following changes were made to bring the build up to date with current tooling.
>
> **Build system updates**
> - `chisel/build.sbt`: migrated from Chisel 3.1 + Scala 2.11 to Chisel 3.6.1 + Scala 2.13.
>   The old `chisel-iotesters` dependency was dropped as it is no longer available on Maven Central
>   and is not required for Verilog generation.
> - `chisel/project/build.properties`: pinned sbt to 1.9.9 for Scala 2.x compatibility.
>
> **Source changes**
> - Entry points in `CoreMem.scala`, `DelayedCapture.scala`, `SpiSlaveIF.scala`, and `SpiSlaveRegs.scala`
>   were updated from the removed `chisel3.Driver.execute` API to `chisel3.stage.ChiselStage.emitVerilog`,
>   which is the standard approach in Chisel 3.5 and later.
